import { MapPin, Clock, User, Phone, Navigation, Camera, FileText } from "lucide-react";
import { Button } from "./ui/button";
import { Textarea } from "./ui/textarea";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "./ui/select";
import { useState } from "react";

interface PnpActiveIncidentScreenProps {
  onNavigate: (screen: string, data?: any) => void;
}

export function PnpActiveIncidentScreen({ onNavigate }: PnpActiveIncidentScreenProps) {
  const [updateNote, setUpdateNote] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Mock active incident data
  const incident = {
    id: "PNP-004",
    type: "Crime",
    subtype: "Robbery in Progress",
  ...
  const [progressStatus, setProgressStatus] = useState("en-route");

  const handleMarkResolved = () => {
    // Update status to resolved and go back
    ...
    <div className="flex flex-col h-full bg-gray-100 pb-16">
      ...
      <div className="bg-blue-600 text-white p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="bg-red-500 px-3 py-1 rounded-full text-sm animate-pulse">
            {incident.priority} Priority
          </div>
        </div>
        <p className="text-sm text-blue-100">{incident.id}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div>
            ...
            <Select value={progressStatus} onValueChange={setProgressStatus}>
              ...
              <SelectContent>
                ...
                <SelectItem value="suspect-apprehended">Suspect Apprehended</SelectItem>
                <SelectItem value="evidence-collected">Evidence Collected</SelectItem>
        ...
        <div className="bg-white rounded-lg p-4 space-y-3">
          ...
          <div>
            <label className="block mb-2">Status Update / Notes</label>
            <Textarea
              placeholder="Enter current status, actions taken, or notes..."
              value={updateNote}
          ...
          <Button
            ...
            disabled={!updateNote || !progressStatus || isSubmitting}
